...
def load_vote_margins(path: str) -> dict[str, dict]:
    ...
    # (key, vote_date, motion_text) identifies one roll call.
    ...
    for key, evs in by_bill.items():
        ...
        latest = sorted(evs, key=lambda e: e["vote_date"])[-1]
        tallies[key] = {
...
def bill_mentioned_in(identifier: str, descriptor: str) -> bool:
    ...
    pat = re.compile(rf"\b{re.escape(prefix)}\s*0*{re.escape(number)}\b",
                     re.IGNORECASE)
