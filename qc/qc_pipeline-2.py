...
def check_status_vocab(record: dict) -> list[Issue]:
    status = text(record, "Status").strip().lower()
    if not status:
        return []
    if status not in _CANONICAL_STATUS:
        return [Issue("MEDIUM", "STATUS_VOCAB", "Status",
                      f"Status '{status}' is outside the canonical vocabulary. Normalize it on ingest.")]
    return []


def check_action_complete_status_logic(record: dict) -> list[Issue]:
    status = text(record, "Status").strip().lower()
    ac_raw = text(record, "action_complete").strip().lower()
    if not status or not ac_raw:
        return []
    if ac_raw not in {"true", "false", "1", "0", "yes", "no"}:
        return [Issue("LOW", "ACTION_COMPLETE_PARSE", "action_complete",
                      f"action_complete has non-boolean value '{ac_raw}'.")]
    ac = ac_raw in {"true", "1", "yes"}
    if status in _RESOLVED_STATUS and not ac:
        return [Issue("MEDIUM", "ACTION_COMPLETE_MISMATCH", "action_complete",
                      f"Status '{status}' is terminal but action_complete=False. Set action_complete=True.")]
    if status in _INPROGRESS_STATUS and ac:
        return [Issue("MEDIUM", "ACTION_COMPLETE_MISMATCH", "action_complete",
                      f"Status '{status}' is in-progress but action_complete=True. Set action_complete=False.")]
    return []
...
BASELINE_CHECKS = [
    ...
    check_coordinates,
    check_status_vocab,
    check_action_complete_status_logic,
