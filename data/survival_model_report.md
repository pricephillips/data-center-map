...
Generated 2026-09-14 by `survival_model.py`. Figures re-derived from current CSVs at generation time.
...
- Opposed projects in the model: **6** (3 reached a terminal decision = events; 3 still pending = right-censored).
- Of the 3 events: 2 `advanced_confirmed`, 1 `blocked_confirmed`.
...
- 0 opposed projects were EXCLUDED from the time axis because their announcement date is only year-precision (too coarse to floor without fabricating months).
...
- Median time to a `advanced` decision: 492 days.
...
WITHHELD: only 3 events (< 25 minimum). A Cox model on this few events would be unstable; KM above is the defensible summary until more decisions resolve.

## Limitations (binding)

- 3 events is a small basis for survival estimates; treat all numbers as provisional and interval-wide.
