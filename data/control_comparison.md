...
Generated 2026-07-15 by `control_comparison.py`. All figures re-derived from the current CSVs at generation time.
...
- Opposed projects (treatment side): **179**, of which 82 decided / 97 pending
...
- Matched: **179** opposed projects × k=3 → 537 match rows
...
**all tiers** (537 match rows)
- County 2024 margin: opposed mean -0.125, control mean -0.128, SMD 0.012 — well balanced (n pairs: 519)
...
**proposals_unopposed** (444 match rows)
- County 2024 margin: opposed mean -0.146, control mean -0.148, SMD 0.006 — well balanced (n pairs: 428)
...
- Opposed projects sit in counties with mean 2024 margin -0.125 (n=173); the eligible control pool mean is 0.035 (n=1426).
...
Of **82** decided + opposed projects:
- `advanced_confirmed`: 56 (68%)
...
- Announced-date precision of these rows: month: 25. Month-precision announced dates are floored to the 1st, so those delays carry up to ~30 days of error each.
- `advanced_confirmed` (n=6): 75–492 days, median 294.
...
- Tier usage across all matches: proposals_unopposed: 444, ai_centers: 1, atlas: 92.
