...
Generated 2026-09-02 by `outcome_model.py`. All figures re-derived from current CSVs at generation time; the exact feature matrix is in `outcome_model_features.csv`.
...
- Decided + opposed projects: **87** (61 advanced, 26 `blocked_confirmed`; base rate of blocked = 0.30)
...
- ROC-AUC across folds: **0.85** (10th–90th pct: 0.65–0.93). Chance = 0.50.
- Brier score across folds: **0.167** (10th–90th pct: 0.101–0.250). Predicting the base rate for everyone scores 0.210; lower is better.
...
- Predicted 0.00-0.33: 49 projects; mean predicted 0.13, observed blocked share 0.12
- Predicted 0.33-0.67: 17 projects; mean predicted 0.48, observed blocked share 0.29
- Predicted 0.67-1.00: 21 projects; mean predicted 0.85, observed blocked share 0.71
...
- `mech_public_comment`: +0.193 (coef -1.41, toward advanced)
- `hyperscaler_involved`: +0.038 (coef -0.61, toward advanced)
- `capacity_missing`: +0.026 (coef +0.59, toward blocked)
- `log10_capacity_mw`: -0.025 (coef -0.04, toward advanced)
- `county_margin_2024`: +0.019 (coef +0.54, toward blocked)
- `days_to_first_opposition`: +0.011 (coef -0.38, toward advanced)
- `n_opposition_groups`: -0.009 (coef -0.12, toward advanced)
- `mech_legislation`: -0.009 (coef -0.08, toward advanced)
...
- n=87 with 26 blocked cases; estimates are unstable by nature. Growing the seed via link triage and date recovery is the highest-leverage improvement.
