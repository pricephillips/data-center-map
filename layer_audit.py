...
GITATTRIBUTES = os.path.join(HERE, ".gitattributes")

GITATTRIBUTES_HEADER = """\
# GENERATED by layer_audit.py --write-gitattributes. Do not edit by hand.
#
# Why this file exists. Every path below is written by a module in this
# repository and regenerated from its inputs, and every one of them is also
# committed, because the HTML surfaces fetch them from the repo at runtime.
# Those two facts together produced a standing tax: two branches that both ran
# the pipeline rewrote the same derived files, so any pair of active branches
# conflicted on them within a run or two, whatever their actual changes were.
#
# `-merge` does not stop the conflict. It stops the CORRUPTION. Without it git
# writes <<<<<<< markers into the file, which turns a committed .json or .csv
# that a page fetches into invalid content, and a half-resolved merge can ship
# that. With it, git leaves OUR version intact and marks the path conflicted,
# so the worst case is a stale artifact rather than a broken one. The fix is
# then mechanical: scripts/resolve_generated_conflicts.py regenerates them.
#
# `linguist-generated=true` collapses these in pull request diffs, so a review
# shows the change that was authored rather than thousands of lines of
# regenerated output.
#
# The list is DERIVED, never hand-maintained. layer_audit.py already resolves
# every module's write targets with an AST walk, so this file is regenerated
# from that same map and cannot drift as modules are added. A hand-kept list
# would go stale the first time someone added a writer, which is the exact
# class of defect the layer audit exists to catch.
"""


def not_regenerable(config: dict | None = None) -> set[str]:
    """Paths a module writes but cannot rebuild from scratch.

    Declared in configs/layers.json. Append-only files and sources of record
    belong here: for them a three-way merge is exactly right, because two
    branches appending different rows is a merge git performs correctly.
    """
    if config is None:
        with open(CONFIG, encoding="utf-8") as fh:
            config = json.load(fh)
    return {k for k in config.get("not_regenerable", {})
            if not k.startswith("_")}


def generated_paths(root: str = HERE, config: dict | None = None) -> list[str]:
    """Every committed path a module can rebuild from scratch, sorted.

    Two exclusions, both deliberate:

    Runtime-computed targets (a path built from a variable) resolve to a
    directory or a glob rather than one file, and a pattern that over-matches
    would silence a real merge on a hand-maintained file.

    Declared non-regenerable files are excluded because marking them would be
    a regression rather than a fix. `-merge` is right for a derived artifact
    precisely because a clean three-way merge of two independent regenerations
    produces a file neither run generated. An append-only file is the opposite
    case: merging two append streams is correct, and forcing a conflict there
    would cost work for nothing.
    """
...
def render_gitattributes(paths: list[str]) -> str:
    ...
    for p in paths:
        # Escape spaces so a path with one still matches as a single pattern.
        lines.append(f"{p.replace(' ', '[[:space:]]')} -merge linguist-generated=true")
    return "\n".join(lines) + "\n"


def write_gitattributes(root: str = HERE) -> tuple[int, bool]:
    """Regenerate .gitattributes. Returns (path count, changed)."""
    paths = generated_paths(root)
    text = render_gitattributes(paths)
    target = os.path.join(root, ".gitattributes")
    prior = ""
    if os.path.exists(target):
        with open(target, encoding="utf-8") as fh:
            prior = fh.read()
    if prior == text:
        return len(paths), False
...
def main() -> int:
    ...
    ap.add_argument("--list-generated", action="store_true",
                    help="print every committed path a module writes")
    ap.add_argument("--write-gitattributes", action="store_true",
    ...
    if args.list_generated:
        try:
            ...
            # Piping into head/grep closes the reader early. That is the
            # caller ending the stream, not a failure of this module.
            try:
                sys.stdout.close()
            except BrokenPipeError:
                pass
    ...
    if args.write_gitattributes:
        n, changed = write_gitattributes()
        print(f".gitattributes: {n} generated path(s), "
              f"{'updated' if changed else 'already current'}")
        return 0

    if args.check_gitattributes:
        paths = generated_paths()
        want = render_gitattributes(paths)
        have = ""
        if os.path.exists(GITATTRIBUTES):
            with open(GITATTRIBUTES, encoding="utf-8") as fh:
                have = fh.read()
        if have == want:
            print(f".gitattributes is current ({len(paths)} generated paths)")
            return 0
        print(".gitattributes is STALE. Run: python layer_audit.py "
              "--write-gitattributes")
        return 1
