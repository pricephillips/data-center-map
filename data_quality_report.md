...
1632 valid; 0 non-empty but still non-URL (flagged for review)
...
3 blank State value(s) and 10 blank County value(s) recovered from the Incident/Summary text (conservative: blanks only, never overwrites). Removes 'Unknown state' dashboard buckets for real events.
...
1418 distinct projects identified; 137 span multiple rows; 29 row(s) unified by manual cross-venue override. Largest clusters: xai_colossus (29), port_washington_wi (9), prince_william_county_va (8), reno_nv (6), dona_ana_county_nm (5). Heuristic = location_name + state, plus PROJECT_OVERRIDES for cross-venue projects.
...
353 individual value fixes recorded in `change_log.csv` (columns: row, field, before, after).