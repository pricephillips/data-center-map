...
1507 valid; 0 non-empty but still non-URL (flagged for review)
...
0 blank State value(s) and 4 blank County value(s) recovered from the Incident/Summary text (conservative: blanks only, never overwrites). Removes 'Unknown state' dashboard buckets for real events.
...
1302 distinct projects identified; 129 span multiple rows; 29 row(s) unified by manual cross-venue override. Largest clusters: xai_colossus (29), port_washington_wi (9), prince_william_county_va (8), reno_nv (6), dona_ana_county_nm (5). Heuristic = location_name + state, plus PROJECT_OVERRIDES for cross-venue projects.
...
268 legislative record(s) staged via the gate's ladder (now reading the Status field too); status_clean corrected on 50 record(s) so committee/one-chamber actions aren't labelled enacted; 4 record(s) flagged outcome_overstated (claims success but only at committee/one chamber — the 'approved ≠ law' trap).
...
344 individual value fixes recorded in `change_log.csv` (columns: row, field, before, after).