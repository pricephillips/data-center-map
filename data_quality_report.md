...
1426 distinct projects identified; 138 span multiple rows; 29 row(s) unified by manual cross-venue override. Largest clusters: xai_colossus (29), port_washington_wi (9), prince_william_county_va (8), reno_nv (6), dona_ana_county_nm (5). Heuristic = location_name + state, plus PROJECT_OVERRIDES for cross-venue projects.
...
282 legislative record(s) staged via the gate's ladder (now reading the Status field too); status_clean corrected on 50 record(s) so committee/one-chamber actions aren't labelled enacted; 4 record(s) flagged outcome_overstated (claims success but only at committee/one chamber — the 'approved ≠ law' trap).
