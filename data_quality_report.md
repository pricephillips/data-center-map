...
1820 valid; 0 non-empty but still non-URL (flagged for review)
...
7 blank State value(s) and 12 blank County value(s) recovered from the Incident/Summary text (conservative: blanks only, never overwrites). Removes 'Unknown state' dashboard buckets for real events.
...
299 legislative record(s) staged via the gate's ladder (now reading the Status field too); status_clean corrected on 51 record(s) so committee/one-chamber actions aren't labelled enacted; 4 record(s) flagged outcome_overstated (claims success but only at committee/one chamber — the 'approved ≠ law' trap).
...
359 individual value fixes recorded in `change_log.csv` (columns: row, field, before, after).