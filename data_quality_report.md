...
1643 valid; 0 non-empty but still non-URL (flagged for review)

**4. Sources — backfilled from Source URL where empty**  
273 row(s) now have a populated Sources list (Source URL was confirmed == Sources[0] in 100% of dual-filled rows)
...
78 blank State value(s) and 45 blank County value(s) recovered from the Incident/Summary text (conservative: blanks only, never overwrites). Removes 'Unknown state' dashboard buckets for real events.
...
1458 distinct projects identified; 116 span multiple rows; 29 row(s) unified by manual cross-venue override. Largest clusters: xai_colossus (29), port_washington_wi (9), prince_william_county_va (8), reno_nv (6), fort_worth_tx (4). Heuristic = location_name + state, plus PROJECT_OVERRIDES for cross-venue projects.

**11. Date enrichment: action_year + date_parseable + data_era (new columns)**  
273 unparseable date(s) flagged; 5 row(s) tagged crypto_era_pre2022 (e.g. the lone 2014 Chelan County PUD record) so the two opposition waves can be analyzed separately
...
487 individual value fixes recorded in `change_log.csv` (columns: row, field, before, after).