...
1747 valid; 0 non-empty but still non-URL (flagged for review)
...
1463 distinct projects identified; 159 span multiple rows; 29 row(s) unified by manual cross-venue override. Largest clusters: xai_colossus (29), port_washington_wi (9), prince_william_county_va (8), garfield_township_issues_moratorium_on_data_centers_unknown (8), to_receive_ai_data_center_petition_from_save_lubbock_tx (7). Heuristic = location_name + state, plus PROJECT_OVERRIDES for cross-venue projects.
...
293 legislative record(s) staged via the gate's ladder (now reading the Status field too); status_clean corrected on 50 record(s) so committee/one-chamber actions aren't labelled enacted; 4 record(s) flagged outcome_overstated (claims success but only at committee/one chamber — the 'approved ≠ law' trap).
